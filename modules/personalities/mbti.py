...
async def mbti(app: Ariadne, group: Group, event: GroupMessage, message: MessageChain = DetectPrefix("!mbti")):
        ...
        if str(message) != "":
            await app.send_message(
                ...
                MessageChain([
                    ...
                    Plain(
                        f"数据来源于 https://www.16personalities.com/ch/{str(message).strip().lower()}-%E4%BA%BA%E6%A0%BC"
                        if str(message).strip().lower() in mbti_list else "")
                ])
            )
        else:
